...
class Jira {
			...
			var req = protocolAdapters[this.protocol].request(request, (res) => {
				...
				res.on('end', () => {
					...
					try {
						...
						if(typeof json.errorMessages !== 'undefined' && json.errorMessages.length > 0) {
							reject(`Error: ${json.errorMessages}`)
						}

						if(typeof json.errors !== 'undefined' && Object.keys(json.errors).length > 0) {
	...
	updateIssue(issue, update) {
		return this._doRequest(this._buildRequest({
			path: `/issue/${issue}`
			, method: 'PUT'
		}), update);
	}
	...
	getStatusDurations(issue, unit = 'seconds', includeWeekends = false) {
		return this.getIssueTransitionHistory(issue)
			.then(transitions => {
				...
				for(let status of statuses) {
					...
					statusDurations[status.status] = moment.duration(statusDurations[status.status], 'seconds').as(unit);
				}
